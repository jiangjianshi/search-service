...
import com.huifenqi.search.context.dto.request.house.HousePublishDto;
import com.huifenqi.search.context.dto.request.house.RoomPublishDto;
...
import com.huifenqi.search.context.dto.response.house.ImgLink;
import com.huifenqi.search.context.dto.response.house.RoomQueryDto;
import com.huifenqi.search.context.dto.response.house.SettingVo;
import com.huifenqi.search.context.entity.house.HouseBase;
import com.huifenqi.search.context.entity.house.HouseDetail;
import com.huifenqi.search.context.entity.house.HousePicture;
import com.huifenqi.search.context.entity.house.HouseSetting;
import com.huifenqi.search.context.entity.house.RoomBase;
...
import com.huifenqi.search.context.enums.HouseTagsEnum;
import com.huifenqi.search.context.enums.bd.RoomTypeEnum;
import com.huifenqi.search.context.enums.bd.SettingsEnum;
...
public class HouseUtil {
	...
	public static HouseSearchResultInfo getHouseSearchResultInfo(HouseSolrResult solrResult,
		...
		List<String> webImgs = solrResult.getWebImgs();
        String picWeb = StringUtil.EMPTY;
        if (CollectionUtils.isNotEmpty(webImgs)) {
            picWeb = webImgs.get(0); // TODO 设置图片
        }
        info.setWebPic(picWeb);
		...
		info.setTextscore(String.valueOf(solrResult.getTextscore()));
        info.setFinalscore(String.valueOf(solrResult.getFinalscore()));		
	...
	public static HouseSearchResultInfo getHouseSearchResultInfo(RoomSolrResult solrResult,
		...
		info.setCompanyId(solrResult.getCompanyId());// 公寓ID
		// 添加收藏标识
		if (CollectionUtils.isNotEmpty(collectIdList) && collectIdList.contains(String.valueOf(solrResult.getId()))) {
			info.setCollectFlag(1);
		} else {
			info.setCollectFlag(0);
		}
	...
	public static HouseQueryDto getHouseQueryDto(HouseSolrResult solrResult, int communityHouseCount,
		...
		if(StringUtils.isNotEmpty(comment) && comment.trim().equals("无")){
	 ...
	 * @return
	 */
	public static List<RoomQueryDto> getNewReferHouse(List<RoomSolrResult> rsrList) {
		if (rsrList.isEmpty()) {
			return null;
		}
		...
		try {
			for (RoomSolrResult roomSolrResult: rsrList) {

				// 程序或者图片审核失败，不展示该套内房间
				Integer roomApproveStatus = roomSolrResult.getRoomApproveStatus();
				if (roomApproveStatus != 1 && roomApproveStatus != 3) {
					continue;
				}

				RoomQueryDto roomQueryDto = new RoomQueryDto();
				roomQueryDto.setSellId(roomSolrResult.getSellId());

				long roomId = roomSolrResult.getId();
				roomQueryDto.setRoomId(roomId);
				
				

				String name = roomSolrResult.getRoomName();
				Integer type = roomSolrResult.getRoomType();
				if (type != null) {
					roomQueryDto.setRoomType(RoomTypeEnum.getRoomTypeDesc(type));
				}

				Integer price = roomSolrResult.getPrice();
				if (price != null) {
					roomQueryDto.setRentPriceMonth(price);
				}

				Integer orientation = roomSolrResult.getOrientations();
				if (orientation != null) {
					String orientationsStr = HouseUtil.getOrientationsStr(orientation);
					roomQueryDto.setOrientations(orientationsStr);
				}

				Double area = roomSolrResult.getArea();
				if (area != null) {
				...
				Integer status = roomSolrResult.getStatus();
				if (status != null) {
					roomQueryDto.setStatus(status);
					roomQueryDto.setStatusDesc(Constants.HouseBase.getSimplifiedStatusDesc(status));
				}
				...
				Integer depositMonth = roomSolrResult.getDepositMonth();
				Integer periodMonth = roomSolrResult.getPeriodMonth();
				if (depositMonth != null && depositMonth != null) {
					String paytypeStr = HouseUtil.getPaytypeStr(depositMonth, periodMonth);
					roomQueryDto.setPayType(paytypeStr);
				}

				Integer toilet = roomSolrResult.getToilet();
				if (toilet != null) {
					roomQueryDto.setToilet(toilet);
				}

				Integer balcony = roomSolrResult.getBalcony();
				if (balcony != null) {
					roomQueryDto.setBalcony(balcony);
				}
