...
public class DateUtil extends DateUtils {
	 ...
	 * @return
	 */
	public static int getDiffDaytime(String targetDate) {
		Date tarDate = DateUtil.parse(PATTERN_DATETIME, targetDate);  
        Date nowDate = DateUtil.parse(PATTERN_DATETIME, DateUtil.format(PATTERN_DATETIME, new Date()));
        Calendar cal = Calendar.getInstance();
        cal.setTime(tarDate);
        long time1 = cal.getTimeInMillis();        
        cal.setTime(nowDate);
	...
	{
		String time="2018-02-23 19:09:46";
		int diffday=getDiffDaytime(time);
