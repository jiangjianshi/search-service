...
* 
*/
package com.huifenqi.search.dao.repository.platform;
...
public interface PlatformCustomerRepository extends JpaRepository<PlatformCustomer, Long> {
   ...
   @Query("select a from PlatformCustomer a" + " where a.isWeight="+Constants.platform.IS_IMG_YES+"and a.isDelete="
            + Constants.Common.STATE_IS_DELETE_NO)
    List<PlatformCustomer> findByisWeight();
